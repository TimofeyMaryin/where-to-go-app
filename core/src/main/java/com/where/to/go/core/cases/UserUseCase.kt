...
class UserUseCase: UserService {
    ...
    override suspend fun findUser(email: RestorePasswordModel): Response<UserDto?> {
    ...
    override suspend fun updateUser(id: Int, userDomain: UserDto, authToken: String): Response<UserDto> {
        return RetrofitClient.userService.updateUser(id, userDomain, authToken)
    }
}